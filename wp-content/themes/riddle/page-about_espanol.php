<?php 

/* Template Name: About Espanol */

get_header(); ?>


<div id="main_trigger" class="main">
	
	
	<div class="about_wrapper">
		
		
		<h1>
			
			<span class="about_topline">Inicio</span><!-- about_topline -->
			
			<span class="about_bottomline">Riddle & Brantley</span><!-- about_bottomline -->
			
		</h1>
		
		
	</div><!-- about_wrapper -->
	
	<section id="about_section">
		
		<div class="about_img">
		
			<img src="<?php bloginfo('template_directory');?>/images/intl_about_img_01_desktop.jpg"/>
		
		</div><!-- about_img -->
		
		<div class="about_content_one content">
			
			<span class="intro">Nuestra firma representa personas lesionadas debido a la negligencia de otros en todo tipo de accidentes. También ayudamos a personas que han sido heridas en el trabajo y tenemos un departamento para personas solicitando seguro social por incapacidad y beneficios para veteranos. Tome un momento para explorar nuestro sitio en línea, conózcanos y aprenda como les podemos ayudar. Por favor mándenos un email o llámenos si quiere más información o si quiere discutir su caso.</span><!-- intro -->
			
			<h2>Bienvenidos a la página de internet de nuestra firma.</h2>

			<p>Disfrute nuestro sitio en línea y aquí está una lista corta de nuestras áreas de práctica destacadas (mire todas):</p>
			
			<ul class="columns">
				<li>Accidentes de Auto</li>
				<li>Accidentes Automovilísticos</li>
				<li>Lesiones Personales Serias</li>
				<li>Drogas Defectuosas o<br/> Dispositivos Médicos Peligrosos</li>
				<li>Accidentes de Camión/Tráiler</li>
				<li>Accidentes de Peatón o Ciclista</li>
				<li>Muerte Injusta</li>
				<li>Resbalón y Caída/ Responsabilidad<br/> de Premisas</li>
				<li>Compensación Para Trabajadores</li>
				<li>Seguro Social por Incapacidad</li>
				<li>Beneficios de Veteranos</li>
				<li>Ataques de Animal y Mordeduras de Perro</li>
			</ul>

			
		</div><!-- about_content_one -->
		
		</section><!-- about_section -->

		
		<section class="about_section_two">
			
			<div class="about_content_two content">
				
				<h2>Abogados de Lesiones Personales de Carolina del Norte</h2>

				<p>Lesiones por accidentes que fueron causados por el descuido de otros pueden tener efectos devastadores para una familia entera. En el bufete de Riddle & Brantley, LLP, usamos nuestra experiencia legal combinada de más de 160 años para trabajar por resultados que pueden aliviar un poco el estrés causado por una herida.</p>

<p>También tenemos recursos extensivos para ayudar con su reclamación. Nuestra firma tiene oficiales de policía retirados de la Carolina del Norte que pueden ayudar con la investigación y reunir evidencia que podría ayudarnos a construir un caso sucesivo. Nuestros abogados de lesiones personales expertos de Carolina del Norte representan clientes con una variedad de asuntos relacionados a lesiones personales:</p>

<ul class="columns">
	<li>Herida en Sitio de Construcción</li>
	<li>Drogas y Dispositivos Peligrosos</li>
	<li>Mordeduras de Perro y Animal</li>
	<li>Mesotelioma y Asbestos</li>
	<li>Abuso en Hogar en Ancianos</li>
	<li>Litigación Farmacéutica</li>
	<li>Responsabilidad de Premisas</li>
	<li>Responsabilidad de Productos</li>
	<li>Heridas de Resbalón y Caída</li>
	<li>Accidentes Vehiculares</li>
</ul>

<h2>Accidente Juristas</h2> 
<p>Nuestros abogados y personales conocen bien los aspectos de la ley y hacemos un gran esfuerzo para nuestros clientes. Nos esforzamos en lograr la mejor solución que es la más adecuada para su situación única.</p>

<p>Nuestro equipo quiere asegurarse de que usted entienda la naturaleza de su caso antes de proceder con cualquier acción. Explicaremos los hechos mientras contestamos sus preguntas honestamente para que usted pueda hacer una decisión informada sobre su caso.
Los abogados de accidentes automovilísticos en Riddle & Brantley, LLP tienen más de 160 años de experiencia combinada que incluye, pero no está limitado a:</p>

<ul class="columns">
<li>Accidentes de Peatones y Ciclistas</li>
<li>Accidentes de Carro</li>
<li>Accidentes por Conductores Distractos</li>
<li>Accidentes por Conductores Borrachos</li>
<li>Accidente Por Frente</li>
<li>Accidente de Golpe y Fuga</li>
<li>Accidente Automovilístico</li>
<li>Accidente de Motocicleta</li>
<li>Accidente Posterior</li>
<li>Accidente por Conducción Temeraria</li>
<li>Accidente Volcadura</li>
<li>Herida de Cinturón</li>
<li>Accidente por Exceso Velocidad</li>
<li>Accidente de Textear Mientras Conduce</li>
<li>Accidente Camiónero</li>
</ul>


<h2>Abogados de Compensación Para Trabajadores</h2>

<p>¿Sufrió una herida en el trabajo? ¿No puede hacer sus deberes en el trabajo? Si esto suena como usted, entonces podría ser elegible para recibir compensación para trabajadores. Nuestra firma de ley luchara por usted y sus derechos para asegurar de que usted reciba compensación por su caso. Les asistiremos a través del proceso paso-a -paso que incluye: investigando y solicitando beneficios hasta que sean concedidos, apelar la decisión o incluso volver a instaurar su reclamo.</p>

<p>¡El abogado de compensación para trabajadores, Chris Brantley es un especialista certificado en Carolina del Norte en compensación para trabajadores y examina todos los casos!</p>

<p>También hemos reunido una lista de cuestiones frecuentes y usted puede mirar esas cuestiones aquí en nuestro FAQ sobre Compensación de Trabajadores.
Usted tiene derechos y opciones – ejércelos con uno de nuestros abogados de compensación para trabajadores.</p>
				
			</div><!-- about_content_two -->
			
			<div class="about_img_two">
				
				
				<img class="img_one" src="<?php bloginfo('template_directory');?>/images/intl_about_img_02_desktop.jpg"/>
				
				<img class="img_two" src="<?php bloginfo('template_directory');?>/images/intl_about_img_03_desktop.jpg"/>
				
				<div class="img_content content">
					
					<h2>Abogados de Seguro Social por Incapacidad</h2>

					<p>Si usted o un ser querido está luchando para recibir beneficios de seguro social por incapacidad, nuestros abogados le pueden ayudar a ejercer sus derechos y explorar sus opciones legales. Para poder calificar para seguro social por incapacidad un individuo tiene que tener una condición médica que cumpla con la definición de incapacidad de la Oficina de Seguro Social.</p>
					
					<p>Los abogados de incapacidad de Riddle & Brantley, LLP conocen el sistema de seguro social y pueden luchar por los beneficios que merece.
Tal vez encuentre nuestra lista de preguntas frecuentes sobre el seguro social por incapacidad útil:</p>

	<ul class="columns">
		<li>¿Qué es Seguro Social por Incapacidad?</li>
		<li>¿Qué es Una Incapacidad?</li>
		<li>¿Cómo es Determinada la Incapacidad?</li>
		<li>¿Cuánto Tiempo se Demora en Tomar Una Decisión?</li>
		<li>¿Puedo Recibir Compensación de Trabajador o Seguro Social por Incapacidad a la misma vez?</li>
	</ul>
	
	<h2>Juristas de Beneficios Para Veteranos</h2>

<p>¡En Riddle & Brantley, LLP, cada uno de nuestros abogados tiene miembros de familia que han servido en las fuerzas armadas, y creemos que veteranos incapacitados deben ser intitulados a recibir los beneficios que se merecen!</p>

<p>Nuestro equipo trabajará sin descanso a su nombre, para lograr los beneficios máximo que usted merece. Con ubicaciones de oficinas en Goldsboro, Raleigh, Kinston y Jacksonville, Carolina del Norte, nuestros abogados pueden servir en todo el estado, y en otros lugares necesarios.¡Para más información, les invitamos a llamarnos al <a href="tel:800-525-7111">800-525-7111</a>, o complete el formulario en esta pagina para una Evaluación de Caso Gratuita!</p>
					
				</div><!-- img_content -->
				
			</div><!-- about_img_two -->
			
			
		</section><!-- about_section_two -->

		
		<section id="about_section_es">
			
			
			<div class="about_section_es_image">
				
				<img src="<?php bloginfo('template_directory');?>/images/intl_spanish_img_04.jpg"/>
				
			</div><!-- about_section_es_image -->
			
			<div class="about_section_es_content content">
				
				<h2>Contacte los Abogados de Lesiones en Riddle & Brantley, LLP</h2>

				<p>Entendemos que unas víctimas no pueden desplazarse después de ser lesionado debido a la negligencia de otros, y es por eso que las oficinas de Riddle & Brantley, LLP están orgullosos de hacer visitas de hospital u hogar.</p>
				
				<p>¡Con nosotros representándolo y a su lado, tiene más de 160 años de experiencia, un equipo de investigadores y asistentes legales en su esquina!</p>

				<p>Contacte cualquiera de nuestras 4 ubicaciones a través de Carolina del Norte para una Evaluación de Caso GRATUITA.</p>
				
				<p>¡Déjenos usar nuestro conocimiento, experiencia y recursos para ayudarlo a obtener los resultados que merece! ¡Llámenos o llene nuestro formulario hoy para una Evaluación de Caso GRATUITA – es estrictamente confidencial!</p>
				
				
				<div class="four_columns">
					
					<div class="four_col">
						
						<h3>Raleigh, NC</h3>
						
						<ul>
							
							<li>Apex</li>
							<li>Cary</li>
							<li>Garner</li>
							<li>Holly Springs</li>
							<li>Knightdale</li>
							<li>Morrisville</li>
							<li>Rolesville</li>
							<li>Wake Forest</li>
							<li>Wendell</li>
							<li>Zebulon</li>
							<li>y las áreas alrededort</li>
							
						</ul>
						
					</div><!-- four_col -->
					
					<div class="four_col">
						
						<h3>Jacksonville, NC</h3>
						
						<ul>
							
							<li>Holly Ridge</li>
							<li>Maysville</li>
							<li>North Topsail Beach</li>
							<li>Richlands</li>
							<li>Surf City</li>
							<li>Topsail Beach</li>
							<li> y las áreas alrededort</li>
							
						</ul>
						
					</div><!-- four_col -->
					
					<div class="four_col">
						
						<h3>Goldsboro, NC</h3>
						
						<ul>
							
							<li>Eureka</li>
							<li>Fremont</li>
							<li>Mount Olive</li>
							<li>Pikeville</li>
							<li>Seven Springs</li>
							<li>Walnut Creek</li>
							<li>y las áreas alrededort</li>
							
						</ul>
						
					</div><!-- four_col -->
					
					
					<div class="four_col">
						
						<h3>Kinston, NC</h3>
						
						<ul>
							
							<li>Deep Run</li>
							<li>Grifton</li>
							<li>La Grange</li>
							<li>Pink Hill</li>
							<li>y las áreas alrededort</li>			
						</ul>
						
					</div><!-- four_col -->
					
					
					
				</div><!-- four_columns -->
				
				
			</div><!-- about_section_es_content -->
			
			
		</section><!-- about_section_es -->
	
</div><!-- main -->



<?php get_footer(); ?>





	